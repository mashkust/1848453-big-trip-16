...
export const filter = {
  ...
  [FilterType.FUTURE]: (points) => points.slice().filter((point) => new Date(point.dateFrom) >= new Date() || (new Date(point.dateFrom) < new Date() && new Date(point.dateTo) > new Date())),
  [FilterType.PAST]: (points) => points.slice().filter((point) => new Date(point.dateTo) <= new Date() || (new Date(point.dateFrom) < new Date() && new Date(point.dateTo) > new Date()))
};
...
const createTimeStats = (points) => [...new Set(points.map((point) => point.type))]
  .map((type) => {
    let sum = 0;
    const filterType = points.filter((point) => point.type === type);
    sum = filterType.reduce((acc, index) => acc + (dayjs(index.dateTo).diff(index.dateFrom)),0) ;
